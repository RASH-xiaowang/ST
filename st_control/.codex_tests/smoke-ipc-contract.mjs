...
for (const { file, line, cmd, keys } of calls) {
  ...
  const norm = keys.map(camelToSnake).sort();
  const want = [...rargs].sort();
  audited++;
  if (JSON.stringify(norm) !== JSON.stringify(want)) {
    problems.push({ file, line, cmd, want, got: norm });
  }
}
...
console.log('✓ 全部一致（含 Tauri camelCase→snake_case 自动转换）');
process.exit(0);
